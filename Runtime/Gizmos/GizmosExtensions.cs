...
public static class GizmosExtensions {
    ...
    public static void DrawWireArc(Vector3 center, float radius, float angle, int segments, Quaternion rotation)
	{
		...
		var step = Mathf.Max(1, Mathf.RoundToInt(angle / segments));
	...
	public static void DrawWireCapsule(Vector3 center, float radius, float height) => DrawWireCapsule(center, radius, height, default);
        ...
        public static void DrawCubeExtended(Vector3 position, Quaternion rotation, Vector3 scale)
        {
            ...
            Matrix4x4 oldGizmosMatrix = UnityEngine.Gizmos.matrix;

            UnityEngine.Gizmos.matrix *= cubeTransform;

            UnityEngine.Gizmos.DrawCube(Vector3.zero, Vector3.one);

            UnityEngine.Gizmos.matrix = oldGizmosMatrix;
        }
        ...
        public static void DrawCameraOrthographic(Camera camera, Color gizmoColor, bool showVertex)
        {
            ...
            UnityEngine.Gizmos.DrawLine(x0y0, x1y0);
            UnityEngine.Gizmos.DrawLine(x1y0, x0y1);
            UnityEngine.Gizmos.DrawLine(x1y1, x0y1);
            UnityEngine.Gizmos.DrawLine(x0y0, x1y1);

            if (showVertex)
            {
                UnityEngine.Gizmos.DrawSphere(x0y0, 0.1f);
                UnityEngine.Gizmos.DrawSphere(x1y0, 0.1f);
                UnityEngine.Gizmos.DrawSphere(x0y1, 0.1f);
                UnityEngine.Gizmos.DrawSphere(x1y1, 0.1f);
            }
