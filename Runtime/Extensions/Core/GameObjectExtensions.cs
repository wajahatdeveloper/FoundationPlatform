...
public static class GameObjectExtensions
{
    ...
    public struct AncestorsEnumerable : IEnumerable<GameObject>
    {
        ...
        /// <summary>Store element into the buffer, return number is size. array is automaticaly expanded.</summary>
        public int ToArrayNonAlloc<TState, T>(Func<GameObject, TState> let, Func<TState, bool> filter,
            Func<TState, T> selector, ref T[] array)
        {
            var index = 0;
            var e = this.GetEnumerator(); // does not need to call Dispose.
            while (e.MoveNext())
...
#if UNITY_EDITOR
            List<T> _componentCache; // per-instance scratch buffer (was static; static corrupted overlapping enumerations of same T)
            // Lazy property because struct field initializers require C# 10; this keeps C# 9 compat
            // while still giving each enumerator instance its own buffer.
            List<T> componentCache => _componentCache ?? (_componentCache = new List<T>());
#endif
...
#if UNITY_EDITOR
                _componentCache = null; // lazily created on first use
#endif
            }
    ...
    public struct DescendantsEnumerable : IEnumerable<GameObject>
    {
        ...
        void DescendantsCore(ref Func<GameObject, bool> filter, ref Transform transform, ref int index,
        ...
        void DescendantsCore<T>(ref Func<GameObject, bool> filter, ref Func<GameObject, T> selector,
            ref Transform transform, ref int index, ref T[] array)
        {
            if (!descendIntoChildren(transform)) return;
        ...
        void DescendantsCore<TState, T>(ref Func<GameObject, TState> let, ref Func<TState, bool> filter,
            ref Func<TState, T> selector, ref Transform transform, ref int index, ref T[] array)
        {
            if (!descendIntoChildren(transform)) return;
        ...
        public struct Enumerator : IEnumerator<GameObject>
        {
            ...
            internal Enumerator(Transform originTransform, bool withSelf, bool canRun,
                InternalUnsafeRefStack sharedStack, Func<Transform, bool> descendIntoChildren)
            {
    ...
    /// <summary>
    /// Gets the prefab name from a GameObject, preferring the prefab asset name in editor.
    /// Falls back to GameObject name with "(Clone)" suffix removed.
    /// Returns "Unknown" if GameObject is null.
    /// </summary>
    /// <param name="gameObject">The GameObject to get the prefab name from.</param>
    /// <returns>The prefab name or cleaned GameObject name.</returns>
    public static string GetPrefabName(this GameObject gameObject)
    {
        ...
        return gameObject.name.RemoveCloneSuffix();
    }
