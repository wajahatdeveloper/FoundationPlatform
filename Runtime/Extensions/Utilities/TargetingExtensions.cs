...
public static class TargetingExtensions
{
    ...
    public static GameObject FindNearestUnit(this GameObject go, LayerMask maskToSearch)
    {
        ...
        GameObject[] NearbyGameObjects = new GameObject[NearbyUnits.Length];
        for (int i = 0; i < NearbyUnits.Length; i++)
            NearbyGameObjects[i] = NearbyUnits[i].gameObject;
        return go.FindNearestUnit_List(NearbyGameObjects);
    }
    ...
    public static GameObject FindNearestUnit_View(this GameObject go, float frustumAngle, LayerMask maskToSearch)
    {
        ...
        for (int i = 0; i < NearbyUnits.Length; i++)
            NearbyGameObjects[i] = NearbyUnits[i].gameObject;
        return go.FindNearestUnit_ListView(frustumAngle, NearbyGameObjects);
    }
    ...
    /// <param name="frustumAngle">angle at which we can see the unit</param>
    /// <returns></returns>
    public static GameObject FindNearestUnit_ListRangeView(this GameObject go, float targetRange, float frustumAngle,
        GameObject[] goList)
    {
        // get the forward and position, so we're not calling on each loop iteration
        Vector3 GoPos = go.transform.position;
