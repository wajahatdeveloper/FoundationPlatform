...
using System.IO;
using System.Linq;
using AetherNexus.FoundationPlatform.Editor.Tools.EditorGUIX_ImageStringConverter;
using UnityEditor;
using UnityEngine;
...
public static class EditorGUIX
{
		...
		{
			var guiState = GUI.enabled;
			if (elementAt < 0 || elementAt >= property.arraySize) GUI.enabled = false;
			...
			string externalImportFolder = null) where T : Object
		{
			...
			if (allowExternal && anyExternal)
			{
				...
				foreach (var importedFile in importedFiles)
				{
					...
					{
						result.Add(asset);
						DebugX.Debug("Asset imported at path: " + importedFile);
					}
