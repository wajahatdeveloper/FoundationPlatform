...
namespace AetherNexus.FoundationPlatform.Editor.Utilities
{
	public static class SerializedPropertyExtensions
	{
		...
		/// <summary>
		/// Get FieldInfo out of SerializedProperty. Walks nested/array property paths
		/// (e.g. "foo.bar" or "list.Array.data[2]") down to the leaf field rather than
		/// assuming propertyPath is a single field name on the target type.
		/// </summary>
		public static FieldInfo GetFieldInfo(this SerializedProperty property)
		{
			...
			while (type != null)
			{
				var fieldInfo = type.GetField(leaf, BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance);
		...
		public static void SetValue(this SerializedProperty property,object value)
		{
			...
			// Set on the immediate parent container (which is the target object for
			// top-level fields), not always the root target object — otherwise nested
			// paths would target the wrong instance or throw.
			var target = property.GetParent() ?? property.serializedObject.targetObject;
