...
- **`DebugXLogging` namespace leftover** — `Editor/DebugX/DebugXMenuItems.cs` and everything in `Editor/Console/` still use `DebugXLogging` / `DebugXLogging.ConsoleView.Editor`, while `Runtime/DebugX/` was moved to `FoundationPlatform.DebugX`. Pick one: either fold these into `FoundationPlatform.DebugX.Editor`, or (if `DebugXLogging` is meant to stay as a deliberate in-house brand name — it was, historically) move the Runtime side back to match. Don't leave it split. (Runtime-side string/reflection matching against this namespace — `CallerInfoHelper`, `ExplicitErrorDedupe`, `UnityConsoleStackFormatter` — was fixed 2026-07-07 to match `FoundationPlatform.DebugX`; only the Editor namespace itself is still split.)
- **No `Tests/` asmdef** — zero automated test coverage on this package.
