...
- `Editor/Validation/` — three unrelated concerns, not one pipeline:
  ...
  - `UI/*` (`UIValidationEngine`, `UIValidationConventions`, `UIValidationPolicy`, `UIValidationPostprocessor`, `UIValidationMenu`, `UIValidationReporter`) — a self-contained linter that enforces UI folder-layer conventions (UIElement/Widget/Panel/Orchestration) for scripts and prefabs under `Assets/Scripts/UI`, `Assets/Data/UI`, wired to asset-import batching.
