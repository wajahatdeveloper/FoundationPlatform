// The Framework.Inspector attribute set — in-house drop-in replacements for the third-party
// inspector attributes this project used before. Names and constructor/field signatures mirror the
// original API so historical call sites (~1,800) needed only a using swap. These are pure metadata;
// rendering lives in the Framework.Inspector.Editor drawer engine (FrameworkEditor).
//
// Attributes UnityEngine already provides ([Tooltip], [Range], [TextArea], [Multiline], [Header],
// [Space], [HideInInspector], [Delayed]) are intentionally NOT redefined here — they resolve from
// UnityEngine after the using swap.
